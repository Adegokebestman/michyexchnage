	/* Using the sweet alert plugin. Requires jQuery */
	...
	/* Set a delay of 200 milliseconds to allow calling another pop-up immediately when one closes. */

	function cue_alert(title, message, func)
	{
	   swal({title:title, text:message, showCancelButton: false, confirmButtonText: 'OK', closeOnConfirm: true }, function(s){ setTimeout(function(){ func != null ? func(s): null; }, 200); } );
	}

	function cue_confirm(title, message, func)
	{
	   swal({title:title, text:message, showCancelButton: true, confirmButtonText: 'OK', closeOnConfirm: true  }, function(s){ setTimeout(function(){ func != null ? func(s): null; }, 200); } );
	}


    $('a.cue-btn').click(function(e)
    {

        var button   = $(this); 
        var subject  = $.trim(button.attr('subject' ));
        var confirm  = $.trim(button.attr('confirm' ));
        var callback = $.trim(button.attr('callback'));

        console.log(button.attr('status'));

        if(button.attr('status') == undefined)
        {
            cue_confirm(subject, confirm, function(s)
            {
                if(callback != '') { window[callback](s); } if(s === true) { button.attr('status', true); button[0].click(); button.removeAttr('status'); }
    ...
    $('form .cue-btn[type="button"]').click(function(e) {
        
        var button = $(this);

        if(button.attr('type') == 'submit') { return; } /* prevent stack loop */

        var form = button.closest('form'); if(!form.length) { return; }

        if(!form[0].checkValidity())
        {
            button.attr('type', 'submit'); button.click(); setTimeout(function(){ button.attr('type', 'button'); }, 100); 
        }
        else
        {
            var subject  = $.trim(button.attr('subject' ));
            var confirm  = $.trim(button.attr('confirm' ));
            var callback = $.trim(button.attr('callback'));
            var validate = $.trim(button.attr('validate'));

            if(validate != '' && !window[validate]()) { return; }

            if(subject == '') { subject = 'Submit'; }

            if(confirm == '')
            {
                if(callback != '') { window[callback](); } button.attr('type', 'submit'); button.click();
            }
            else
            {
                cue_confirm(subject, confirm, function(s)
                {
                    if(callback != '') { window[callback](s); } if(s === true) { button.attr('type', 'submit'); button.click(); }
                });
            }
        }
